package pe.edu.upc.university.utils;
...
import pe.edu.upc.university.security.MyUserDetails;
		...
		try {
			...
			if(!(authentication instanceof AnonymousAuthenticationToken)) {	// Si Hay alguien autenticado
				...
				if (myUserDetails.getSegment().equals(Segment.STUDENT)) {
					if (studentService.existsById(myUserDetails.getIdSegment())) {
				...
				// Para el segmento 2: Cambiar TEACHER por su segmento
					...
					if (teacherService.existsById(myUserDetails.getIdSegment())) {
